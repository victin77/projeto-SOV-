...
let currentUser = JSON.parse(localStorage.getItem('racon_current_user')) || null;

// Migrar leads antigos que não têm consultor
function migrateLeads() {
    let needsSave = false;
    leads.forEach(lead => {
        if (!lead.consultor) {
            lead.consultor = 'Admin'; // Leads antigos ficam com Admin
            needsSave = true;
        }
    });
    if (needsSave) {
...
function showApp() {
    ...
    if (userNameEl) {
        const role = currentUser.role === 'admin' ? 'Admin' : currentUser.name;
        userNameEl.textContent = role;
    }
    
    // Mostrar/ocultar campo de consultor baseado no role
    const consultorField = document.getElementById('consultor-field-container');
    if (consultorField) {
        consultorField.style.display = currentUser.role === 'admin' ? 'block' : 'none';
    }
...
function handleLogin(e) {
    ...
    if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
        ...
        localStorage.setItem('racon_current_user', JSON.stringify(currentUser));
        showApp();
        return;
    }
    
    // Verificar consultores (email simples: nome@racon.com, senha: nome123)
    const consultor = CONSULTORES.find(c => 
        email.toLowerCase() === `${c.toLowerCase()}@racon.com` && 
        password === `${c.toLowerCase()}123`
    );
...
function logout() {
    if (confirm('Deseja realmente sair?')) {
        currentUser = null;
        localStorage.removeItem('racon_current_user');
        showLoginScreen();
    }
}
...
function getFilteredLeads() {
    if (currentUser.role === 'admin') {
        ...
        return leads.filter(lead => lead.consultor === currentUser.name);
    }
}
...
function renderApp() {
    ...
    // Popular select de consultores se for admin
    if (currentUser.role === 'admin') {
        const consultorSelect = document.getElementById('nl-consultor');
        if (consultorSelect) {
            consultorSelect.innerHTML = '<option value="">Selecione o consultor</option>' +
                CONSULTORES.map(c => `<option value="${c}">${c}</option>`).join('');
        }
    }
...
function renderKanban(container) {
    ...
    STAGES.forEach(stage => {
        ...
        const filtered = filteredLeads.filter(l => l.stage === stage && leadMatchesFilter(l));
        ...
        filtered.forEach(lead => {
            ...
            const consultorBadge = currentUser.role === 'admin' && lead.consultor 
                ? `<div class="consultor-badge">${lead.consultor}</div>` 
                : '';
...
function renderDashboard(container) {
    const filteredLeads = getFilteredLeads();
    const totalValue = filteredLeads.reduce((acc, curr) => acc + Number(curr.value), 0);
    const wonLeads = filteredLeads.filter(l => l.stage === 'Fechado');
    const lostLeads = filteredLeads.filter(l => l.stage === 'Perdido');
    const totalWon = wonLeads.reduce((acc, curr) => acc + Number(curr.value), 0);
    const activeLeads = filteredLeads.filter(l => l.stage !== 'Fechado' && l.stage !== 'Perdido');
    const conversion = filteredLeads.length > 0 ? ((wonLeads.length / filteredLeads.length) * 100).toFixed(1) : '0.0';
            ...
            <div class="dash-table">
                ...
                ${wonLeads.length === 0 ? '<p>Nenhuma venda fechada ainda.</p>' : wonLeads.slice(-5).reverse().map(l => `
                    <div class="table-item">
                        <div>
                            <span>${l.name}</span>
                            ${currentUser.role === 'admin' && l.consultor ? `<div class="consultor-badge" style="margin-top: 4px;">${l.consultor}</div>` : ''}
                        </div>
...
function addNewLead() {
    ...
    const consultorField = document.getElementById('nl-consultor');
    const consultor = currentUser.role === 'admin' 
        ? (consultorField.value || currentUser.name)
        : currentUser.name;
    ...
    document.getElementById('nl-value').value = '';
    document.getElementById('nl-step').value = '';
    if (consultorField) consultorField.value = '';
        ...
        <div class="modal-card edit-card">
            ...
            <div class="edit-body">
                <div class="edit-main">
                    ...
                    <select id="ed-consultor">
                        ${CONSULTORES.map(c => `<option value="${c}" ${lead.consultor === c ? 'selected' : ''}>${c}</option>`).join('')}
                    </select>
                    ` : ''}
...
function saveEdit(id) {
    ...
    // Atualizar consultor se for admin
    if (currentUser.role === 'admin') {
        const consultorSelect = document.getElementById('ed-consultor');
        if (consultorSelect) {
            lead.consultor = consultorSelect.value;
        }
    }
