...
const KANBAN_BATCH_SIZE = 4;
let kanbanVisibleByStage = Object.fromEntries(STAGES.map((stage) => [stage, KANBAN_BATCH_SIZE]));
let kanbanShowAll = false;
let kanbanActiveTab = localStorage.getItem('sov_kanban_tab') || 'all';
...
// --- KANBAN ---
function ensureKanbanVisibilityState() {
    if (kanbanActiveTab !== 'all' && !STAGES.includes(kanbanActiveTab)) {
        kanbanActiveTab = 'all';
    }
    for (const stage of STAGES) {
        const curr = Number(kanbanVisibleByStage[stage]);
        if (!Number.isFinite(curr) || curr < KANBAN_BATCH_SIZE) {
            kanbanVisibleByStage[stage] = KANBAN_BATCH_SIZE;
        }
    }
}

function setKanbanTab(tab) {
    const next = tab === 'all' || STAGES.includes(tab) ? tab : 'all';
    if (kanbanActiveTab === next) return;
    kanbanActiveTab = next;
    kanbanShowAll = false;
    localStorage.setItem('sov_kanban_tab', next);
    renderApp();
}

function showMoreKanbanLeads() {
    ensureKanbanVisibilityState();
    kanbanShowAll = false;
    if (kanbanActiveTab === 'all') {
        for (const stage of STAGES) {
            kanbanVisibleByStage[stage] += KANBAN_BATCH_SIZE;
        }
    } else {
        kanbanVisibleByStage[kanbanActiveTab] = (kanbanVisibleByStage[kanbanActiveTab] || KANBAN_BATCH_SIZE) + KANBAN_BATCH_SIZE;
    }
    renderApp();
}

function showAllKanbanLeads() {
    kanbanShowAll = true;
    renderApp();
}
...
function renderKanban(container) {
    ...
    STAGES.forEach((stage) => {
        ...
        const limit = kanbanShowAll ? Number.MAX_SAFE_INTEGER : (kanbanVisibleByStage[stage] || KANBAN_BATCH_SIZE);
        const shown = filtered.slice(0, limit);
        const hidden = Math.max(0, filtered.length - shown.length);

        stagesData[stage] = { filtered, shown, hidden };
        totalFiltered += filtered.length;
    });

    const tabs = document.createElement('div');
    tabs.className = 'kanban-tabs';
    const tabsData = [{ value: 'all', label: 'Geral', count: totalFiltered }]
        .concat(STAGES.map((stage) => ({
            value: stage,
            label: stage,
            count: (stagesData[stage] && stagesData[stage].filtered ? stagesData[stage].filtered.length : 0)
        })));

    for (const tab of tabsData) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = `kanban-tab ${kanbanActiveTab === tab.value ? 'active' : ''}`;
        btn.textContent = tab.label;
        btn.onclick = () => setKanbanTab(tab.value);
    ...
    actions.className = 'kanban-controls-actions';

    const btnMore = document.createElement('button');
    btnMore.type = 'button';
    btnMore.className = 'btn-small btn-secondary';
    btnMore.textContent = 'Mostrar mais';
    btnMore.onclick = showMoreKanbanLeads;

    const btnAll = document.createElement('button');
    btnAll.type = 'button';
    btnAll.className = 'btn-small btn-secondary';
    btnAll.textContent = 'Mostrar tudo';
    btnAll.onclick = showAllKanbanLeads;
    ...
    if (kanbanActiveTab === 'all') {
        ...
        STAGES.forEach((stage) => {
            ...
            const stageData = stagesData[stage] || { filtered: [], shown: [], hidden: 0 };
            const filtered = stageData.filtered;
            const shown = stageData.shown;
            const hidden = stageData.hidden;
            ...
            if (hidden > 0) {
                ...
                hint.className = 'col-footer-hint';
                hint.textContent = `+${hidden} lead(s) oculto(s)`;
        ...
        if (hasHidden) container.appendChild(controls);
        return;
    }

    const stageData = stagesData[kanbanActiveTab] || { filtered: [], shown: [], hidden: 0 };
    const stageView = document.createElement('div');
    stageView.className = 'kanban-stage-view';
    stageView.innerHTML = `
        <div class="stage-view-header">
            <h3>${kanbanActiveTab}</h3>
            <span>${stageData.filtered.length} lead(s)</span>
        </div>
    `;
    ...
    if (!stageData.filtered.length) {
        const empty = document.createElement('div');
        empty.className = 'kanban-stage-empty';
        empty.textContent = 'Nenhum lead nessa etapa com os filtros atuais.';
        grid.appendChild(empty);
    } else {
        stageData.shown.forEach((lead) => {
    ...
    if (stageData.hidden > 0) container.appendChild(controls);
}
